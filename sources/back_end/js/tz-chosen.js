(function($) {
    return $.fn.tzChosen = function(options) {
        ...
        $var.updater    = function(item){
            ...
            $(this).width(25);
            cursource[cursource.indexOf(item)]='';
        ...
        var _html    = '<div id="tz_chosen_'+id+'" class="chzn-container chzn-container-multi">';
        _html += '<ul class="tz_chzn-choices chzn-choices"><li class="search-field">';
        _html += '</li></ul></div>';

        $(_html).insertBefore($(this));

        var main    = $('#tz_chosen_'+id);
        ...
        var addElement  = function(index,value){
            ...
            html += '<input type="hidden" name="'+inputName+'" value="'+value.trim()+'"/>';
            html += '</li>';
        ...
        main.find('.search-choice-close').live('click',function(){
            ...
            if(medsource.indexOf(nitem.find('input').attr('value')) != -1){
        ...
        if($var.sourceEdit){
            $.each($var.sourceEdit,function(index,value){
                addElement(cursource.indexOf(value),value);
                cursource[cursource.indexOf(value)]='';
            });
            $(this).width(25);
        }
            ...
            if(eval(strKeysEqual) && e.which != 13){
                if($('.typeahead').css('display') != 'none'){
                    ...
                    if($('.typeahead').find('li').hasClass('active')){
                        ...
                        if(newsource.indexOf(cval) != -1){
                            addElement(cursource.indexOf(cval),cval);
                            $(this).width(25);
                            cursource[cursource.indexOf(cval)]='';
                        }
                    }
                }
                e.preventDefault();
