    ...
    function tzSortFilter(srcObj, desObj, order) {
        ...
        srcObj.sort(function (a, b) {
            ...
            if (jQuery(a).attr('data-option-value') != '*' &&
                ...
                if (typeof order !== "undefined" && order.substr(order.length - 3, order.length).toLowerCase() == 'asc') {
                    return (compA < compB) ? -1 : (compA > compB) ? 1 : 0;
                }
                if (typeof order !== "undefined" && order.substr(order.length - 4, order.length).toLowerCase() == 'desc') {
                    return (compA > compB) ? -1 : (compA < compB) ? 1 : 0;
                }
            }
        });
